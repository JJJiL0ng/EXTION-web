...
const QuickActionGrid: React.FC = () => {
  const quickActions = [
    { 
      ...
      color: 'bg-gray-50 border-blue-400 hover:bg-blue-50',
      accent: 'text-[#005de9]'
    },
    { 
      title: '자연어로 함수 요청', 
  ...
  return (
    <div className="mb-4">
      <h2 className="text-xl font-semibold text-gray-900 mb-4">이런 걸 할 수 있어요. 눌러서 실행해보세요.</h2>
      <div className="grid grid-cols-4 gap-3">
        {quickActions.map((action, index) => (
          <Link href="/ai" key={index}>
            <div className={`p-4 rounded-xl border-2 cursor-pointer transition-all hover:scale-105 ${action.color}`}>
