...
// Context 생성
const TargetSheetContext = createContext<SpreadsheetContextType | null>(null);
...
// Provider 컴포넌트
export const TargetSheetProvider: React.FC<SpreadsheetProviderProps> = ({
  children,
  spreadRef
}) => {
  ...
  return (
    <TargetSheetContext.Provider value={contextValue}>
      {children}
    </TargetSheetContext.Provider>
  );
};

// Context 사용 Hook
export const useTargetSheetContext = (): SpreadsheetContextType => {
  const context = useContext(TargetSheetContext);

  if (!context) {
    throw new Error('useTargetSheetContext must be used within a TargetSheetProvider');
  }
...
export const useTargetSheetContextSafe = (): SpreadsheetContextType | null => {
  return useContext(TargetSheetContext);
};

export { TargetSheetContext };
