...
// 채팅 모드 타입 정의
export type ChatMode = 'normal' | 'formula' | 'datafix' | 'artifact';
...
export async function determineChatMode(
  ...
  try {
    const systemPrompt = `
      ...
      1. normal: 일반적인 대화나 간단한 질문, 시트 조작이 필요없을때 사용(ex: 안녕, 데이터 분석 어떻게 해야 할지 모르겠어요)
      2. formula: 스프레드시트 함수가 필요한 작업 요청(ex: 합계, 평균, 최대, 최소 등)
      3. datafix: 데이터 정렬, 필터링, 전체 데이터의 변환이나 수정 혹은 데이터의 추가가 필요할 떄 사용(ex: 오름차순 정렬, 중복 제거, 특정 값 수정, 데이터 지우기)
